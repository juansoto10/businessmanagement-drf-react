...
const SmallSetPagination = ({ get_items_list_page, items_list, count }) => {
  ...
  const previous_number = () => {
    ...
    if (currentPage > 1) {
      ...
      get_items_list_page(currentPage - 1)
  ...
  const getNumbers = () => {
    ...
    for (let i = 0; i < count; i += itemsPerPage) {
      ...
      if (active === page) {
        ...
        content = (
          <div key={i} onClick={() => visitPage(page)} className={`hidden sm:flex sm:-mt-px`}>
